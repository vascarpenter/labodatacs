...
namespace labodatacs
{
    ...
    public partial class MainWindow : Window
    {
         ...
         */
        public void ReformatText(String text, out Paragraph para, out String plain)
        {
            String t = text;
            String t0 = "";
            String t1 = "";
            ...
            for (int i = 0; i < ilen;)
            {
                ...
                { // ignore "<0.1" but skip "<r: ...>"
                  ...
                  // flush t0
                    ...
                    if(ch=='r')
                    {
                       span = new Span { Foreground = Brushes.Red };

                    }
                    else if(ch=='b')
                    {
                        span = new Span { Foreground = Brushes.Blue };
                    }
                    i++;
                    do
                    {
        ...
        {
            linefeed = linefeedCheck.IsChecked == true;
            addunit = unitCheck.IsChecked == true;
            IDataObject data = Clipboard.GetDataObject();
            if(data.GetDataPresent(DataFormats.Text))
            {
                string str = (string)data.GetData(DataFormats.Text);
                string outstr = ConvertClipboard();
                // なんらかの処理をする
                ...
                Clipboard.SetDataObject(plain, true);
            }
        }
